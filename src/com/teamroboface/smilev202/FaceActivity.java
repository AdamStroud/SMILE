...
/**Version 2.0.3 (still, for some reason)
 *
 * List of suggestions, bugs, and potential improvements in top drawer of my normal desk (~Elise).
 */
...
import com.teamroboface.smilev202.R;
import com.teamroboface.smilev202.EmotionalFace.Emotion;
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		speakButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				} else if (System.currentTimeMillis() - timeCompletedSpeaking > 750) {		
					startListening(speechRecogNormal);				// this starts the activity for normal listening (i.e. not learning new words)
				} else {
					return;											// don't respond to any touches noticed while app is busy speaking
				}
 				/* ****************************************************************************************/
			}
		});
			...
			if (!topHit.equals("stop")) {
				...
				if (phrasesPresent.size() > 0) {		
					...
					if (phrasesPresent.get(0).equals("learn")) {		
						...
						handler.postDelayed(runnable, 180); // start listening for learn protocol only AFTER animation and tts response are done
					...
					// Keyword "Clear memory" clears saved files of all added keywords, not including hard-coded ones.
						...
						handler.postDelayed(runnable, 180); // start listening again normally after finished speaking.
					...
					// Keyword "Command mode" starts command listener, so that robot can be controlled by voice commands.
						...
						animate(Emotion.HAPPY); //animate to happy
						handler.postDelayed(runnable, 180); // start listening for command protocol only AFTER animation and tts response are done
					...
					// Keyword "Camera mode" sets animation to be only happy, no blinking, until camera mode is exited.
						...
						if (cameraMode) {
							...
							handler.postDelayed(runnable, 180);	// Start listening again when camera mode exited
						} else {
					...
					// All other keywords are dealt with here.
						...
						handler.postDelayed(runnable, 180); // start listening again only after animation and tts response are done
					}
	...
	{		
		...
		public void onResults(Bundle results)                   
		{
			...
			if (!topHit.equals("stop") && !topHit.equals("exit")) {
				...
				new Thread(new ClientThread(commandMessage.toString(), ROBOT_PORT, ROBOT_IP)).start(); //put the command in the socket
				}
				...
				{	
					speakOut(topHit);
					new Thread(new ClientThread(topHit.toString(), ROBOT_PORT, ROBOT_IP)).start(); //put the command in the socket
//					speakOut("Sorry, can you repeat that?");
				}
	...
	class ClientThread implements Runnable  {
		...
		public void run() {
			try {
				...
				packet.setData(messageToSend.toString().getBytes());
				socket.send(packet);
				socket.close();
